...
function attach(ctx) {
  ...
  app.get("/api/demo/accounts", (req, res) => {
    ...
    res.json({ ok: true, demo: true, password: "demo123", otp: "000000", accounts: list });
  });
  ...
  function serializePost(post, viewerId) {
    ...
    return {
      ...
      authorAvatar: (u && u.profileImage && String(u.profileImage).indexOf("data:") !== 0) ? u.profileImage : "",
      text: post.text || "",
  ...
  app.get("/api/feed", auth, (req, res) => {
    ...
    res.json({ ok: true, posts: list.slice(0, 80).map((p) => serializePost(p, req.userId)) });
  });
...
module.exports = { attach, ensureFamilyRoles, can, roleOf, DEFAULT_ROLE_DEFS };
