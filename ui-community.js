...
export async function mountDemoPanel(H, formWrap) {
  const { make, apiTry, setSession, cacheUserFromServer, closeModal, updateUIForSession } = H;
  const box = make("div", { className: "demo-panel" });
  box.appendChild(make("div", { className: "help-kicker" }, "Test APIs · no setup"));
  box.appendChild(make("div", { style: "font-weight:700;margin-bottom:6px" }, "Jump in as a demo family member"));
  const list = make("div", { className: "demo-grid" });
  box.appendChild(list);
  const hint = make("div", { className: "small-muted", style: "margin-top:8px" }, "Password for all: demo123 · OTP: 000000");
  ...
  j.accounts.forEach((a) => {
    ...
    b.appendChild(make("div", { style: "font-weight:800" }, a.name));
    b.appendChild(make("div", { className: "small-muted" }, role + " · " + (a.phone || a.email)));
    b.onclick = async () => {
  ...
  const g = make("button", { className: "google-btn", type: "button", style: "margin-top:10px" }, "Continue with Google (demo)");
  g.onclick = async () => {
    const r = await apiTry("/api/auth/demo-google", { method: "POST", body: {} });
    if (!r || !r.user) return uiNotice("Demo Google is off");
    cacheUserFromServer(r.user);
    setSession({ userId: r.user.id, sessionToken: r.sessionToken });
    closeModal();
    updateUIForSession();
