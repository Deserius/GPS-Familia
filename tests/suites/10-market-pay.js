...
module.exports = {
  ...
  async run(t) {
    ...
    const demoCount = ((demo.json && demo.json.listings) || []).filter((l) => l.demo === true).length;
    t.check("six mock listings for every user", demoCount >= 6, String(demoCount));
    const asBuyer = await t.get("/api/market", { token: buyer.token });
    t.check("mock listings visible to a second account", ((asBuyer.json && asBuyer.json.listings) || []).filter((l) => l.demo === true).length >= 6);
