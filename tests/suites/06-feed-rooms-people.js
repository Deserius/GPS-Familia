...
module.exports = {
  ...
  async run(t) {
    ...
    const outsiderRooms = await t.get("/api/rooms", { token: a.token });
    const catalog = []
      .concat((outsiderRooms.json && outsiderRooms.json.rooms) || [])
      .concat((outsiderRooms.json && outsiderRooms.json.discover) || [])
      .map((r) => r.name);
    const named = ["General", "Welcome", "Sports Talk", "Music", "Movies & TV", "Gaming", "Travel", "Book Club", "Gardening", "Faith", "Fishing", "Parents", "Neighborhood", "Sunday Sauce", "Classic Rides"];
    t.check("named public rooms for every signed-in user", named.filter((n) => catalog.includes(n)).length >= 12, catalog.join(", "));
    t.check("Poker Night stays private", !catalog.includes("Poker Night"));
    ...
    if (pid) {
      ...
      t.check("comment saved on post", !!(cmt.json && cmt.json.post && Array.isArray(cmt.json.post.comments) && cmt.json.post.comments.some((x) => x.text === "Noted.")));
    ...
    const dir = await t.get("/api/search", { token: vito.token });
    await t.expectStatus(dir, 200, "GET /api/search directory");
    t.check("empty search lists people", !!(dir.json && Array.isArray(dir.json.users) && dir.json.users.length >= 1),
      "users=" + ((dir.json && dir.json.users) || []).length);
  }
};
