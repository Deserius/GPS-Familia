...
export async function mountCommunityFeed(H, root, opts) {
  ...
  const state = { posts: [], filter: "all", q: "", pending: [], more: false, offset: 0 };
  ...
  const refreshBtn = make("button", { className: "btn small secondary feed-refresh", type: "button", title: "Refresh" }, "↻");
  ...
  const reqBanner = make("div", { className: "req-banner" });
  reqBanner.hidden = true;
  root.appendChild(reqBanner);

  const stories = make("div", { className: "feed-stories" });
  ...
  const charEl = make("div", { className: "feed-char" }, "0 / 4000");
  ta.addEventListener("input", () => { charEl.textContent = String((ta.value || "").length) + " / 4000"; });
  composer.appendChild(charEl);
  ...
  const chipDefs = [["all", "All"], ["public", "Everyone"], ["family", "Families"], ["mine", "Mine"], ["photos", "Photos"], ["videos", "Videos"]];
  chipDefs.forEach(([id, label]) => {
    const b = make("button", { className: "inbox-chip" + (id === "all" ? " on" : ""), type: "button" }, label);
    b.dataset.id = id;
    b.onclick = () => { state.filter = id; chips.querySelectorAll(".inbox-chip").forEach((x) => x.classList.toggle("on", x.dataset.id === id)); load(true); };
    chips.appendChild(b);
  });
  ...
  function postCard(p) {
    ...
    cwrap.style.display = (p.comments && p.comments.length) ? "block" : "none";
  ...
  function paintStream() {
    ...
    if (!state.posts.length) {
      const empty = make("div", { className: "feed-empty" });
      empty.appendChild(make("div", {}, "No posts in this view yet."));
      empty.appendChild(make("div", { className: "small-muted" }, "Share a photo, a clip, or a note — Everyone or a family."));
      stream.appendChild(empty);
      return;
    }
    state.posts.forEach((p) => stream.appendChild(postCard(p)));
    if (state.more) {
      const more = make("button", { className: "btn small secondary feed-loadmore", type: "button" }, "Load more");
  ...
  async function loadHub() {
    ...
    const famPeople = [];
    const seen = new Set();
    (H.myFamilies() || []).forEach((f) => (f.members || []).forEach((id) => {
      if (seen.has(id) || id === s.userId) return;
      seen.add(id);
      const u = getUserById(id);
      if (u) famPeople.push(u);
    }));
    famPeople.slice(0, 16).forEach((u) => {
      const b = make("button", { className: "feed-story", type: "button" });
      const av = make("div", { className: "chat-avatar" });
      av.innerHTML = getAvatarFor(u);
      b.append(av, make("span", {}, (u.name || "Member").split(" ")[0]));
      b.onclick = () => { q.value = u.name || ""; load(true); };
      stories.appendChild(b);
    });
    const n = hub && hub.counts ? hub.counts.incoming : 0;
    if (n > 0) {
      reqBanner.hidden = false;
      reqBanner.innerHTML = "";
      reqBanner.appendChild(make("div", { style: "flex:1" }, n + " family request" + (n === 1 ? "" : "s") + " waiting"));
      const go = make("button", { className: "btn small", type: "button" }, "Review");
      go.onclick = () => { if (typeof H.closeFeedPanel === "function") H.closeFeedPanel(); if (typeof H.renderFamilyList === "function") H.renderFamilyList("", { tab: "requests" }); };
      reqBanner.appendChild(go);
    } else reqBanner.hidden = true;
  }

  async function load(reset) {
    if (reset !== false) { state.offset = 0; state.posts = []; }
    if (!state.offset) {
      stream.innerHTML = "";
      stream.appendChild(make("div", { className: "small-muted" }, "Loading…"));
    }
    const j = await apiTry("/api/feed?filter=" + encodeURIComponent(state.filter) + "&q=" + encodeURIComponent(q.value.trim()) + "&offset=" + state.offset + "&limit=30");
    const batch = (j && j.posts) || [];
    if (state.offset) state.posts = state.posts.concat(batch);
    else state.posts = batch;
    state.more = !!(j && j.more);
  ...
  let t;
  q.addEventListener("input", () => { clearTimeout(t); t = setTimeout(() => load(true), 220); });
