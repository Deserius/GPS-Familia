...
module.exports = {
  ...
  async run(t) {
    ...
    if (mid) {
      ...
      const goneA = await t.get("/api/messages?with=" + encodeURIComponent(b.user.id), { token: a.token });
      t.check("deleted DM gone for sender", !((goneA.json && goneA.json.messages) || []).some((m) => m.id === mid));
      const goneB = await t.get("/api/messages?with=" + encodeURIComponent(a.user.id), { token: b.token });
      t.check("deleted DM gone for recipient", !((goneB.json && goneB.json.messages) || []).some((m) => m.id === mid));
    }

    const hideMe = await t.post("/api/messages", { to: b.user.id, enc: { iv: "aGlkZQ", ct: "bWVtZQ" } }, { token: a.token });
    const hideId = hideMe.json && hideMe.json.message && hideMe.json.message.id;
    if (hideId) {
      const delForMe = await t.del("/api/messages/" + hideId, {}, { token: b.token });
      await t.expectStatus(delForMe, 200, "recipient deletes DM for me");
      const stillA = await t.get("/api/messages?with=" + encodeURIComponent(b.user.id), { token: a.token });
      t.check("delete-for-me keeps sender copy", ((stillA.json && stillA.json.messages) || []).some((m) => m.id === hideId));
      const hiddenB = await t.get("/api/messages?with=" + encodeURIComponent(a.user.id), { token: b.token });
      t.check("delete-for-me hides recipient copy", !((hiddenB.json && hiddenB.json.messages) || []).some((m) => m.id === hideId));
    }

    const famId = famMsg.json && famMsg.json.message && famMsg.json.message.id;
    if (famId) {
      try {
        const delEvtP = wsB.wait((m) => m.type === "message_deleted" && m.id === famId, 4000);
        const delFam = await t.del("/api/messages/" + famId, {}, { token: a.token });
        await t.expectStatus(delFam, 200, "sender deletes family message for everyone");
        const delEvt = await delEvtP;
        t.check("WS message_deleted to family", !!(delEvt && delEvt.id === famId));
        const famGone = await t.get("/api/messages?family=" + family.id, { token: b.token });
        t.check("family delete gone for other member", !((famGone.json && famGone.json.messages) || []).some((m) => m.id === famId));
      } catch (e) {
        t.fail("family delete / WS", e.message);
      }
    }
