  ...
  const pages = {
    ...
    start: () => {
      ...
      const steps = [
        ...
        ["Invite", "Find people → Invite, or share the QR / link. They Accept or Decline. Location stays off until they belong."],
    ...
    families: () => {
      ...
      d.append(
        ...
        make("p",{}, "Open ☰ or 👥 → Create / Join Family. The card is compact: Family name and Password sit side by side as normal-sized boxes, Privacy + Create on the next row, then Join with an invite token."),
        make("ul",{},
    ...
    search: () => {
      ...
      d.append(
        ...
        make("p",{}, "From a result you can Message or Invite them to a family you pick. Pins stay hidden until they belong."),
        make("ul",{},
          make("li",{}, "People — live directory by name or phone."),
    ...
    faq: () => {
      ...
      d.append(
        ...
        make("p",{}, "After login, tap Allow alerts, then Allow on the phone prompt. On iPhone, add GPS FAMILIA to the Home Screen first. You can retry from Help → Phone alerts."),
...
function renderCreateJoin(prefillInvite){
  ...
  invite.setAttribute("aria-label","Invite token");
...
function renderFamilyList(prefillQ, opts){
  ...
  function syncTabs(){
    ...
    hero.style.display = (tab === "people" || tab === "families") ? "" : "none";
    resultsWrap.style.display = (tab === "people" || tab === "families") ? "" : "none";
    myWrap.style.display = (tab === "families" || tab === "invite") ? "" : "none";
    reqBox.style.display = tab === "requests" ? "" : "none";
  }
  function paintTab(){
    syncTabs();
    if(tab === "people") renderResults(searchInput.value);
    if(tab === "families") renderResults(searchInput.value);
  ...
  async function renderResults(q){
    ...
    if(!query){
      resultsWrap.appendChild(make("div",{className:"small-muted"}, tab === "families" ? "Public families you can join." : "Type a name or phone number to search the directory."));
    }
...
function bindBgLifecycle(){
  ...
  document.addEventListener("visibilitychange", () => {
    ...
    else if(trackingEnabled) pingCurrentPosition();
  });
...
function uiBag(){
  return {
    make, api, apiTry, currentSession, setSession, afterSignIn, cacheUserFromServer, closeModal, updateUIForSession, renderFamilyList, renderAvatarPicker,
...
function mountDebugHud(){
  ...
  const tick = () => {
    el.textContent = "v2.11 · " + (document.hidden ? "hidden" : "live") + " · " + (qs("#live-pill") ? qs("#live-pill").textContent : "");
  };
  tick();
...
function boot(){
  ...
  if(!currentSession()){
    ...
    if(!u){ renderAuth(); }
      ...
      ensureAlertPermission().then(() => resumeOrOfferTracking());
    }
  }
}
